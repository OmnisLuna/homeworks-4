...
class NewsViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let ph = newsCard.copyHistory?[0].attachments?[0].photo?.sizes[3].url
        cell.repostPhoto.image = imageService?.photo(atIndexpath: indexPath, byUrl: ph ?? "")
        
        cell.repostSourceAvatar.image = imageService?.photo(atIndexpath: indexPath, byUrl: newsCard.copyHistory?[0].sourceAvatar ?? "")
        let aspectRatio = (newsCard.copyHistory?[0].attachments?[0].photo?.sizes[3].aspectRatio ?? 0)
